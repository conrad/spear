...
describe('results reducer', () => {
  ...
  it('should handle HIDE_RESULT_OVERLAY', () => {
    const resultIndex: number = 0
    const excerptIndex: number = 0
    // NOTE: Create copy of sample results object in order not to mutate it. 
    const resultsCopy = getCopy(sampleResults)
    ...
    const showingResults = results(resultsCopy, showOverlay({ resultIndex, excerptIndex }))

    expect(
      results(showingResults, hideOverlay())
    ).toEqual(expectedResults)
  })
  ...
  it('should handle EXPORT_RESULTS', () => {
    ...
    expect(
      results(resultsCopy, exportResults())
    ).toEqual(expectedResults)
  })
